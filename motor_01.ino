...
//Setup method - runs when arduino powers on
void setup() {
  Serial.begin(9600);
  Serial.println("Simple Motor Shield sketch");
}

//Loop method - continualy runs
void loop() {
 // Suppose a DC motor is connected to M1_A(+) and M1_B(-)
  ...
  // is ignored.
    ...
    Serial.println("Forward for two seconds");
    motor(1, FORWARD, 255);
    delay(2000);
  
  // Be friendly to the motor: stop it before reverse.
    Serial.println("1/2 second release the motor");
    motor(1, RELEASE, 0);
    delay(500);
    Serial.println("Backwards half speed for 2 seconds");
